...
Ext.ux.grid.GridFilters = Ext.extend(Ext.util.Observable, {
    ...
    getMenuFilter : function () {
        ...
        // Modificado para soporte de filtrar por otro campo (Martin)
        if (view.cm.config[view.hdCtxIndex].filter && view.cm.config[view.hdCtxIndex].filter.dataIndex){
            return this.filters.get(
                view.cm.config[view.hdCtxIndex].filter.dataIndex
            );
        } else {
            return this.filters.get(
        ...
        if (filters) {
            ...
            try {
                for (i = 0, len = filters.length; i < len; i++) {
                    filter = false;
                    if (cm) {
                        // lo modifico para que se pueda setear el dataindex del filtro aunq este definido en el columnmodel
                        //dI = filters[i].dataIndex;
                        
                        if (filters[i].filter) {
                           dI = filters[i].filter.dataIndex || filters[i].dataIndex;
                        } else {
                           dI =  filters[i].dataIndex; 
                        }
