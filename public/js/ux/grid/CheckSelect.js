...
Ext.grid.SmartCheckboxSelectionModel = Ext.extend(Ext.grid.RowSelectionModel, {
     ...
     * @selectionsChanges
     * 
     * Lleva los cambios de las filas seleccionadas o deseleccionadas
     * (Martin)
     */
    selectionsChanges: {},
    
    fieldCheckValues:  null,
    id : 'checker',
    // private
    menuDisabled:true,
    fixed:true,
    ...
    {
       //define header
	   this._headerID = Ext.id();
       this.header = '<div id="'+ (this._headerID) +'" class="x-grid3-hd-checker"> </div>';
        ...
        if (this.grid.store) {
            this.grid.store.on('load', function(p){
                ...
                for(var i = 0, len = count; i < len; i++){
                    ...
                    if((dataIndexValue == 1 || isSelected)){
    ...
    clearSelectionStates: function ()
    {
        //vaciamos el estado
        delete this.selectionsChanges;
        this.selectionsChanges = {};
    },
    
    /**
     * Retorna los cambios en las selecciones
     * @autor Martin A Santangelo
     */
    getSelectionChanges: function () {
    	return this.selectionsChanges;
    },
    /**
    ...
    toggleChecked : function(rowIndex, c) {
        ...
        var valor = record.data[this.dataIndex];
        if (c === undefined) c = !valor;
        record.data[this.dataIndex] = c;
        
		vrow = this.grid.getView().getRow(rowIndex);
		//console.log(vrow);
        var checkCol = Ext.select('.x-grid3-cc-checker', false, vrow);
		...
		if (!this.fieldCheckValues) {
			this.fieldCheckValues = this.grid.store.reader.meta.id;
		}
        sId = record.data[this.fieldCheckValues]+"";
        
        // Guardo los cambios de estados (Martin)
        if (this.ignoreChanges) return;
        if (this.selectionsChanges[sId] !== undefined){
        	delete this.selectionsChanges[sId];
        } else {
      		this.selectionsChanges[sId] = c;
        }
        this.grid.store.fireEvent('update',{});
    },
    ...
    onMouseDown : function(e, t){
        if(t.className && t.className.indexOf('x-grid3-cc-checker') != -1){
            e.stopEvent();
            // Define variables
            var view = this.grid.getView();
    ...
    renderer : function(v, p, record){
        ...
        return '<div class="x-grid3-check-col'+(v?'-on':'')+' x-grid3-cc-checker"> </div>';
    }
}); 
