...
Ext.extend(Ext.ux.grid.GridSummary, Ext.util.Observable, {
    ...
    init : function(grid) {
        ...
        // Lo mande en el onload del store pq sino tiraba error si la grilla estaba dentro de un formulario
        ...
        grid.store.on({
            ...
            load: this.setEventHandler,
            scope: this
        });
        // Agrego refresco de resumen cuando se selecciona una fila (Martin)
        grid.getSelectionModel().on('selectionchange',this.refreshSummary,this);
    ...
    calculate : function(rs, cm) {
        ...
        for (var i = 0, len = cfg.length; i < len; i++) { // loop through all columns in ColumnModel
            ...
            if (cf.summaryType) {
            	isFunction = (typeof cf.summaryType) === 'function'; 
                for (var j = 0, jlen = rs.length; j < jlen; j++) {
                    var r = rs[j]; // get a single Record
                    if (isFunction) {
                    	data[cname] = cf.summaryType(r.get(cname), r, cname, data, j, this.grid);
                    } else {
                    	data[cname] = Ext.ux.grid.GridSummary.Calculations[cf.summaryType](r.get(cname), r, cname, data, j, this.grid); 
                    }
                }
            }
        }
...
Ext.ux.grid.GridSummary.Calculations = {
	checkedsum: function(v, record, colName, data, rowIdx, grid) {
    ...
    selectedsum : function(v, record, colName, data, rowIdx, grid) {
		if (record.data.checked) {
			return data[colName] + Ext.num(v, 0);
		} else {
			return data[colName];
