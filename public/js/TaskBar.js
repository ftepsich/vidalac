...
Ext.extend(Ext.ux.TaskBar, Ext.util.Observable, {
    init : function(){
        ...
        this.startMenu.add(this.app.appsMenu);
        ...
        var startButton = this.startButton =new Ext.Button({
            ...
            template: new Ext.Template(
                ...
                '<td class="ux-startbutton-left"><i>&#160;</i></td><td class="ux-startbutton-center"><button class="x-btn-text {2}" type="{1}" style="height:30px;">{0}</button></td><td class="ux-startbutton-right"><i>&#160;</i></td>',
                '</tr></tbody></table>'
            )
        });
        ...
        var startWidth = Ext.getCmp('ux-startbutton').getWidth() + 10;
...
Ext.ux.TrayPanel = Ext.extend(Ext.BoxComponent, {
    ...
    add : function(config){
        if (config.id) {
            var o = {tag:'li',id: config.id};
        } else {
            var o = {tag:'li'};
        }
        var li = this.strip.createChild(o, this.edge);    // insert before the edge
        var btn = new Ext.Button(Ext.apply(config, {
            cls:'x-btn-icon',
            menuText: config.text,
            renderTo: li,
            text: '', // do not display text
            template: new Ext.Template(
                '<table cellspacing="0" class="x-btn {3} tray-icon"><tbody><tr>',
                '<td class="ux-taskbutton-left"><i>&#160;</i></td>',
                '<td class="ux-taskbutton-center"><em class="{5} unselectable="on">',
                    '<button class="x-btn-text {2}" type="{1}" style="height:28px;">{0}</button>',
                '</em></td>',
