...
{
    /**
     * @var Zend_Cache  adaptador para cacheo del api de las clases
     */
    static public $cache;
    /**
     * @var array   mapa de metodos adicionales no existentes en la clase (capturados a travez de __call)
     */
    static public $addMethods;
    /**
     * @var array   events array ('type', 'data')
     */
     ...
     */
    static public $namespace = 'Models';
    /**
    ...
    static public function get_api_array()
    {
        ...
        foreach (self::$api_classes as $class) {
            if (self::$cache) {
                $actions[$class] = self::$cache->load('Ext_Direct_' . $class);
            }
            if (!@$actions[$class]) {

                $methods = array();

                if (self::$addMethods[$class])
                    $methods = self::$addMethods[$class];
                ...
                if (self::$cache) {
                    self::$cache->save($actions[$class], 'Ext_Direct_' . $class);
                }
            }
        }
...
Ext.Direct.addProvider( [%actions%] );
...
JAVASCRIPT;
    ...
    static public function fire($type, $message)
    {
        self::$events[] = array('type' => $type, 'data' => $message);
    }
...
class ExtDirectAction
{
    ...
    public function run()
    {
        ...
        } catch (Rad_ConfirmationException $e) {
    ...
    protected function call_action()
    {
        ...
        if (( substr($this->method, 0, 2) == '__' ) || (!in_array($this->method, get_class_methods($class)))) {
            if (!in_array(array('name' => $this->method, 'len' => count($this->parameters)), ExtDirect::$addMethods[$class]))
        ...
        if (is_callable(ExtDirect::$authorization_function)) {
            ...
            if ($auth_result === false) {
                if (APPLICATION_ENV != 'production') {
                    throw new Exception('No autorizado ' . $this->action . '::' . $this->method, E_USER_ERROR);
                } else {
...
class ExtDirectController
{
    ...
    public function run()
    {
        if (empty($this->request->actions))
            ...
            foreach (ExtDirect::$events as $event) {
                $msg = array();
                $msg['type'] = 'event';
                $msg['name'] = $event['type'];
                $msg['data'] = $event['data'];
    ...
    public function output()
    {
        ...
        // Fix para que se envien los log a firebug
