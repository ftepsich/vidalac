...
class Facturacion_Model_DbTable_TiposDeComprobantes extends Rad_Db_Table
{
    ...
    {
        $condicion = " (TiposDeComprobantes.Grupo = 6) and TiposDeComprobantes.Id not in (26)";
        $where = $this->_addCondition($where, $condicion);
        return parent:: fetchAll($where, $order, $count, $offset);
    }
    ...
    {
        $condicion = " TiposDeComprobantes.Grupo in(6,7,12) and TiposDeComprobantes.Id not In (26,31,39)";
        $where = $this->_addCondition($where, $condicion);
        return parent:: fetchAll($where, $order, $count, $offset);
    }
