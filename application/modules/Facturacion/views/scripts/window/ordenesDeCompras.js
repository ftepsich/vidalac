...
Apps.<?=$this->name?> = Ext.extend(RadDesktop.Module, {
    ...
    requires: [
      '/direct/Facturacion/OrdenesDeCompras?javascript'
    ],  
    ...
    createEditorWindow: function () {
        ...
        this.grid.abmWindow.on('hide', function(){
                ...
                if (id != 0) {
                    // Actualizo la grilla
                    Models.Facturacion_Model_OrdenesDeComprasMapper.get(id, function(result, e) {
                        if (e.status) {
    ...
    renderPaso3: function () {
        return {
            ...
            buttons : [{
                ...
                handler: function() {
                    var id = this.form.getForm().findField('Id').getValue();
                    Models.Facturacion_Model_OrdenesDeComprasMapper.cerrar(id, function(result, e) {
                        if (e.status) {
                            this.grid.abmWindow.closeAbm();
                        }
                    }, this);
    ...
    /**
     * Boton de anulacion de comprobante
     */
    botonAnular: function ()
    {
        return {
            text: 'Anular',
            ...
            handler: function() {
                ...
                if (selected.get('Cerrado')=='0') {
                    Ext.Msg.show({
                        title: 'Atencion',
                        msg: 'Esta orden de compra aun no esta cerrada.',
                        modal: true,
                        icon: Ext.Msg.WARNING,
                        buttons: Ext.Msg.OK
                    });
                ...
                if (selected.get('Anulado')=='1') {
                    Ext.Msg.show({
                        title: 'Atencion',
                        msg: 'Esta orden de compra ya se encuentra anulada.',
                        modal: true,
                        icon: Ext.Msg.WARNING,
                        buttons: Ext.Msg.OK
                    });
                ...
                if (Ext.Msg.confirm('Atencion','¿Está seguro que desea anular el comprobante seleccionado?',function(btn) {
                    if (btn == 'yes') {
                        ...
                        Models.Facturacion_Model_OrdenesDeComprasMapper.anular(id, Ext.emtpyFn, this);
                        Models.Facturacion_Model_OrdenesDeComprasMapper.get(id, function(result, e) {
                            if (e.status) {
                                this.form.updateGridStoreRecord(result);
                            }
    ...
    /**
     * Boton de envio de orden adjunto por mail
     */
    botonEnviarMail: function() {
        return {
            text: 'Enviar Mail',
            iconCls: 'x-btn-text-icon',
